...
#
#   Some examples:
#   > hubot harvest account save joe@example.org doe
#   > hubot harvest list
#   > hubot harvest start at myproject/important-task: Some notes go here.
#   > hubot harvest start at myp/imp: Some notes go here.
#   > hubot harvest daily of nickofotheruser
#
#   Full command descriptions:
#
#   hubot harvest save <email> <password>
#     Saves your Harvest credentials to allow Hubot to track
#     time for you.
#
#   hubot harvest account delete
...
#
#   hubot harvest start at <project>/<task>: <notes>
...
#
#   hubot harvest stop [at <project>/<task>]
...
#
#   hubot harvest daily [of <user>] [on yyyy-mm-dd]
...
#
#   hubot harvest list [tasks of <user>]
...
module.exports = (robot) ->
  ...
  robot.respond /harvest status/i, (msg) ->
    check_harvest_down (error) ->
      if error
  ...
  robot.respond /harvest account save (.+) (.+)/i, (msg) ->
  ...
  robot.respond /harvest account delete/i, (msg) ->
  ...
  robot.respond /harvest daily( of (\w+))?( on (\d{4})-(\d{2})-(\d{2}))?/i, (msg) ->
  ...
  robot.respond /harvest list( tasks of (.+))?/i, (msg) ->
    unless user = check_user(robot, msg, msg.match[2])
      return
  ...
  robot.respond /harvest start at (.+)\/(.+): (.*)/i, (msg) ->
  ...
  robot.respond /harvest start$/i, (msg) ->
    unless user = check_user(robot, msg)
      return
  ...
  robot.respond /harvest stop( at (.+)\/(.+))?/i, (msg) ->
    unless user = check_user(robot, msg)
      return
